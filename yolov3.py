...
class Yolov3(Darknet53):
    ...
    def graph(self, input):
        ...
        with tf.variable_scope('yolov3'):
            ...
            predictions_2, route = self._detection_block(route_2, 256, self.anchors[3:6], input_size)
            route = self.conv2d_bn(input=route,
                            num_kernels=128,
                            kernel_size=1)
            route = self._upsample(route)
    ...
    def _detection_block(self, input, num_kernels, anchor_list, input_size):
        num_anchors = len(anchor_list)
        objectness = 1
        coordinates=4
        num_classes = 80
        num_predictions = objectness + coordinates + num_classes
        output_depth = num_anchors * num_predictions
        for i in range(3):
        ...
        predictions = tf.reshape(predictions, [-1, grid_dim*num_anchors, num_predictions]) # predictions = (m, 507, 85)
        
        anchors_xy, anchors_hw, confidence, classes = tf.split(predictions, [2, 2, 1, num_classes], axis=-1) # split along the last dimension
        ...
        predictions = tf.concat([anchors_xy, anchors_hw, confidence, classes], axis=-1) # concat along the last dimension
